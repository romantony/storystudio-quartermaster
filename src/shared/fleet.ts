 ...
 *   - The four `workers` sum to ACCOUNT_CAP (10), so it's a *static* allocation,
...
export const FLEET: FleetEndpoint[] = [
  // flux-tts-s2t hosts 8 asset types (t2i, i2i, TTS-kokoro, TTS-qwen, animate,
  // merge, Whisper-SRT, BGM). All models resident in VRAM → a worker switches
  // asset types with no reload cost, but concurrency is still the worker count
  // (4), not the model count — one worker runs one request at a time.
  { counterKey: FLUX_TTS_S2T,    endpointId: 'rnqxi6c0mlq517', workers: 4 },
...
export const PROJECT_FLEET: Record<string, ProjectFleetPlan> = {
  // Basic runs entirely on flux-tts-s2t. Merge is the terminal per-frame step,
  // so its backlog is the cleanest "frames still unfinished" signal.
  'narration-basic': {
    endpoints: [FLUX_TTS_S2T],
    gateEndpoint: FLUX_TTS_S2T,
    gateOperation: 'merge',
    gateMax: 9, // "< 10"
  },
  // Premium touches image (qwen-edit), video (wan2), and audio/merge/SRT/BGM
  // (flux). Wan2 is the bottleneck (90s/job), so it's the gate.
  'narration-premium': {
    endpoints: [QWEN_IMAGE_EDIT, WAN2_I2V, FLUX_TTS_S2T],
