...
1. ~~**Deploy the worker image.**~~ Committed (`0107e90`) and pushed; the Docker Hub build ran from
   `.github/workflows/docker-build.yml`, publishing `romantony/wan2-i2v-4steps:latest` + `:1.0.0`.
   **Still outstanding: recycle the endpoint's workers so they pull the new `:latest`.** Endpoint
   `nd7wloyvj09xwy` → template `kme3t993lu` → `romantony/wan2-i2v-4steps:latest`, `workersStandby: 6`,
   so the 6 warm workers keep serving the OLD image until they are recycled (drain `workersStandby`
   to 0, wait, restore to 6 — or re-save the template in the dashboard). **Per the fixed-pod policy
   this is an admin action via the RunPod dashboard, not something any code here should do**
   (memory `qm-orchestrator-diagnostics-and-fixed-pods-20260917`). Until it happens the pod ignores
   `seed` — harmless, but inert.
...
5. **Decide whether "probation" camera moves should mean anything.** `prepare.ts`'s
   `normalizeContractForCompile` applies `downgrade_move` unconditionally, and that edit downgrades
   `probation` exactly like `banned` — so in practice **only `static` and `push_in` ever reach the
   4-step model**, and `camera.essential: true` (which routes to Replicate instead) is the only way
   to get any other move. V-CAM-02's instruction text does say "banned or on probation … downgrade
   it", so this may well be deliberate; but the two statuses then carry no behavioural difference on
   either video profile, which makes the capability tables misleading. Either give probation real
   meaning (e.g. allow it through and let the QA ladder downgrade on failure) or collapse the two
   statuses. Documented as-is for now in
   `docs/storystudio-narration-premium-prompt-generation.md` §6.
6. **`frames[].shot` extraction quality, when the caller does NOT supply one.** The same dry run had
   GPT-5 mini turn "other students chat in groups" into a *subject* with `count: 4`, compiled as the
   literal noun phrase "four blurred, conversational clusters" — which an image model will render.
   It also called a classroom with other students `sparse`. This is the argument for StoryStudio
   supplying `shot` rather than relying on extraction, but the extraction prompt is worth tightening.
