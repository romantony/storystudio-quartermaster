...
function dialoguePremiumShotMap(opts: {
  ...
  const base = {
    Type: 'Map',
    Comment: 'Per-shot via Quartermaster gateway + RunComfy InfiniteTalk (Dialogue Premium). MaxConcurrency=3 (lowered from 12, 2026-08-17) — RunComfy has no documented account concurrency ceiling anywhere in this codebase (storystudio-dialogue-qm-sfn-implementation.md §5 flagged this as an unaddressed gap: "RunComfy spend ceiling... not implemented... flag if this needs to be built before high-volume Dialogue Premium traffic"), and it just was: a real 163-shot execution at MaxConcurrency=12 saw 32/41 monologue shots (78%) and 8 image:i2i calls time out at qm-generate.ts\'s 850s blocking-poll ceiling — RunComfy\'s own dashboard showed many of those jobs had actually completed, just not before QM gave up waiting. 3 matches dialogue-basic\'s own already-proven-safe RunComfy concurrency (GenerateNarratorSegments\' MaxConcurrency, same physical InfiniteTalk endpoint). This throttles ALL shot kinds (action/narration shots don\'t touch RunComfy at all) since the Map is shared across kinds — a real throughput cost, but correctness over speed given the alternative is silently missing ~1 in 4 shots. Splitting monologue/dialogue into their own lower-concurrency path (so action/narration keep higher throughput) is a real follow-up, not done here. The 850s poll ceiling itself staying too tight for genuinely slow-but-successful RunComfy jobs is ALSO a known, separate, NOT-yet-fixed gap (a job RunComfy eventually completes can still get abandoned+wasted by QM) — out of scope for this specific fix, flagged for a future session.',
    MaxConcurrency: 3,
